        ...
        <div className="row product-style2 g-0">
            <div className="col-lg-6 col-md-12">
                <div className="product-box style-4">
                    <div className="product-media" style={{ backgroundImage: `url(${items[0].image ? (items[0].image.src || getImageUrl(items[0].image)) : IMAGES.ShopLargbnr1.src})` }}></div>
                    <div className="sale-box">
                        <div className="badge style-1 mb-1">{items[0].description || "Sale Up to 50% Off"}</div>
                        <h2 className="sale-name">{items[0].title || "Summer"}<span>{new Date().getFullYear()}</span></h2>
                        <Link href={items[0].buttonLink || "/shop-list"} className="btn btn-outline-secondary btn-lg text-uppercase">{items[0].buttonText || "Shop Now"}</Link>
                    </div>
                </div>
            </div>
            <div className="col-lg-6 col-md-12">
                <div className="product-box style-4">
                    <div className="product-media" style={{ backgroundImage: `url(${items[1].image ? (items[1].image.src || getImageUrl(items[1].image)) : IMAGES.ShopLargbnr2.src})` }}></div>
                    <div className="product-content">
                        ...
                        <Link href={items[1].buttonLink || "/shop-list"} className="btn btn-secondary btn-lg text-uppercase">{items[1].buttonText || "Shop Now"}</Link>
                    </div>
                </div>
            </div>
        </div>
