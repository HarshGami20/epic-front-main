...
export default function VideoSection({ data }: { data?: any }){
    ...
    const bgUrl = typeof data?.backgroundImage === 'string' && data.backgroundImage.trim() 
        ? getImageUrl(data.backgroundImage.trim()) 
        : IMAGES.BackBg2.src;

    const videoSrc = (typeof data?.videoUrl === 'string' && data.videoUrl.trim())
        ? getImageUrl(data.videoUrl.trim())
        : "/assets/images/video.mp4";

    return(
        <Fragment>
            <div className="video-wrapper bg-parallax" style={{backgroundImage:`url('${bgUrl}')`}}>
            ...
            <Modal className="quick-view-modal" show={openVideo} onHide={()=>setOpenVideo(false)} centered>               
                ...
                <div className="modal-body">
                    <video key={videoSrc} width="100%" height="100%" controls autoPlay playsInline>
